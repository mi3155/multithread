...
// multithreadView.cpp: CmultithreadView 클래스의 구현
//
...
BEGIN_MESSAGE_MAP(CmultithreadView, CView)
	...
	ON_COMMAND(ID_32771, &CmultithreadView::OnFileFindmax)
	ON_MESSAGE(WM_MAX, &CmultithreadView::OnMaxFound)
END_MESSAGE_MAP()

// CmultithreadView 생성/소멸
...
CmultithreadDoc* CmultithreadView::GetDocument() const // 디버그되지 않은 버전은 인라인으로 지정됩니다.
{
...
void CmultithreadView::OnFileFindmax(){
	data.array[0] = 1;
	data.array[1] = 2;
	data.array[2] = 3;
	data.array[3] = 4;
...
void CmultithreadView::OnMaxFound(WPARAM wParam, LPARAM lParam)
{
	CClientDC dc(this);
	CString msg;
